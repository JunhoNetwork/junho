...
func (min MinCommissionDecorator) AnteHandle(
	...
	validMsg := func(m sdk.Msg) error {
		...
		case *stakingtypes.MsgCreateValidator:
			...
			if c.Rate.LT(minCommissionRate) {
				return cosmossdkerrors.Wrap(sdkerrors.ErrUnauthorized, "commission can't be lower than 5%")
			}
	...
	validAuthz := func(execMsg *authz.MsgExec) error {
		for _, v := range execMsg.Msgs {
			...
			if err != nil {
				return cosmossdkerrors.Wrapf(sdkerrors.ErrUnauthorized, "cannot unmarshal authz exec msgs")
			}
