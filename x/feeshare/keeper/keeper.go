...
import (
	...
	cosmossdkerrrors "cosmossdk.io/errors"
	...
	revtypes "github.com/JunhoNetwork/junho/x/feeshare/types"
...
func (k Keeper) SendCoinsFromAccountToFeeCollector(ctx sdk.Context, senderAddr sdk.AccAddress, amt sdk.Coins) error {
	if senderAddr.Empty() {
		return cosmossdkerrrors.Wrap(sdkerrors.ErrInvalidAddress, "senderAddr address cannot be empty")
	}
...
func (k Keeper) SendCoinsFromFeeCollectorToAccount(ctx sdk.Context, recipientAddr sdk.AccAddress, amt sdk.Coins) error {
	if recipientAddr.Empty() {
		return cosmossdkerrrors.Wrap(sdkerrors.ErrInvalidAddress, "recipient address cannot be empty")
	}
